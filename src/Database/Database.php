...
class Database {
	...
	private static function connect() {
		if (! static::$connection) {
			//$database_data = File::require_file('config/mysql.php');
			$database_data = File::require_file('config/postgresql.php');
	...
	private static function execute(Array $data, $query, $where = null) {
		...
		// if($query == "INSERT"){VALUES}elseif($query == "UPDATE"){SET}
		// elseif($query == "DELETE"){}
		if ($func == "INSERT") {
			$query .= " (" . static::$setter . ") VALUES (" . trim($var, ', ') . ");";
		} else {
			$query .= static::$setter;
		}
		...
		$db = static::$connection->prepare($query);
		
		$db->execute(static::$binding);
		//print_r($query);
		static::clear();
	}
	...
	public static function insert($data) {
		...
		$object_id = static::$connection->lastInsertId($table.'_id_seq');
	...
	public static function links($current_page, $pages) {
		...
		if ($pages > 1) {
			...
			$links .= "<li class='link $current_page_active'><a href='$href'>Первая</a></li>";
			...
			if ($pages > 1) {
				...
				$links .= "<li class='link $current_page_active'><a href='$href'>Последняя</a></li>";
			}
